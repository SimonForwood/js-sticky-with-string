...
class stickyHelper {
    ...
    createSvg() {
        ...
        this.stickiesElem.insertBefore(svgDiv, this.stickiesElem.firstChild);
    ...
    addSticky(title, desc, color, element, ulElem) {
        if(!ulElem)
        {
        ...
        if(this.hasStrings)
            this.addStickyLine(li, element, color);
    }

    addStickyLine(li, element, color) {
        if(element) {
            var line = document.createElementNS('http://www.w3.org/2000/svg','line');
            line.setAttribute('x1', li.offsetLeft + li.offsetWidth / 2);
            line.setAttribute('y1', li.offsetTop + 10);            
            line.setAttribute('x2', element.offsetLeft + element.offsetWidth / 2);
            line.setAttribute('y2', element.offsetTop + element.offsetHeight / 2);
        ...
        if(stickies && stickies.length > 0) {
            ...
            for(var i=0; i<stickies.length; i++)
                this.addSticky(stickies[i].title ? stickies[i].title : "",
                            ...
                            stickies[i].color,
                            document.getElementById(stickies[i].element),
                            ulElem);
        }
    }

    redraw() {
        if(this.stickiesElem && this.stickiesElem.firstChild) {
            this.stickiesElem.removeChild(this.stickiesElem.querySelector("div.stickWString"));
        }
        this.createSvg();
        for(var i=0; i<this.stickies.length; i++)
            this.addStickyLine(this.stickies[i].li, this.stickies[i].element, this.stickies[i].color);
    }
}
